...
export class Manager extends EventEmitter {
  ...
  private connectionCallbacks: Map<ConnectionEvents, (e: Error) => void> =
    new Map<ConnectionEvents, () => void>();
  ...
  constructor(options: LogstashTransportOptions, connection: IConnection) {
    ...
    this.connectionCallbacks.set(
      ...
      this.onConnectionClosed.bind(this)
    );
    this.connectionCallbacks.set(
      ConnectionEvents.ClosedByServer,
  ...
  private addEventListeners() {
    this.connection.once(
      ConnectionEvents.Connected,
      this.connectionCallbacks.get(ConnectionEvents.Connected)!
    );
    this.connection.once(
      ConnectionEvents.Closed,
      this.connectionCallbacks.get(ConnectionEvents.Closed)!
    );
    this.connection.once(
      ConnectionEvents.ClosedByServer,
      this.connectionCallbacks.get(ConnectionEvents.ClosedByServer)!
    );
    this.connection.once(
      ConnectionEvents.Error,
      this.connectionCallbacks.get(ConnectionEvents.Error)!
    );
    this.connection.once(
      ConnectionEvents.Timeout,
    ...
    this.connection.on(
      ...
      this.connectionCallbacks.get(ConnectionEvents.Drain)!
    );
  }
  ...
  flush() {
    ...
    while (
      ...
      if (logEntry) {
        ...
        const self = this;
        connectionIsDrained = this.connection.send(
          entry + "\n",
          (error?: Error) => {
            if (error) {
